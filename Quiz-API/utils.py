...
from Json_utils import JsonModel
...
def returns_json(handler):
	...
	def wrapper(*args, **kwargs):
		try:
			...
			print_exc()
			ret = APIException.internal_server_error(e)
		if type(ret) is tuple:
			obj, code = ret
		else:
			obj = ret
			code = obj.code if isinstance(obj, APIException) else None
	...
	def decorator(handler):
		...
		def wrapper(*args, **kwargs):
			...
			if content_length is None:
				raise APIException("Missing Content-Length HTTP header", 411)
			if content_length > 10_000_000:
				raise APIException("Maximum allowed payload size is 10 MB", 413)
			...
			try:
				...
				raise APIException(f"Invalid encoding: {e}")
			try:
				if is_list:
				...
				raise APIException(f"Invalid JSON payload: {e}")
