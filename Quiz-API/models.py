...
class Answer(DatabaseModel, JsonModel):
	...
	def validate(self):
		if not self.text:
			raise APIException("Missing answer text")
...
class Question(DatabaseModel, JsonModel):
	...
	def validate(self):
		if not self.text:
			raise APIException("Missing question text")
		if not self.title:
			raise APIException("Missing question title")
		if not self.possible_answers:
			raise APIException("Missing question answers")
		correct_count = 0
		for answer in self.possible_answers:
			...
			if answer.is_correct:
				correct_count += 1
		if correct_count != 1:
			raise APIException("There should be one and only one correct answer to the question")
		if self.position <= 0 or self.position > Question.count() + 1:
			raise APIException("Invalid question position")
...
class LoginRequest(JsonModel):
	...
	def validate(self):
		if not self.password:
			raise APIException("Missing login password")
...
class Participation(JsonModel):
	...
	def validate(self):
		if not self.player_name:
			raise APIException("Missing participant name")
