...
from models import db, Question, LoginRequest, LoginResponse, QuizInfo, Score, Participation, QuestionId, \
	AnswerSummary, ParticipationResponse
from handle_Exception import APIException
...
def handle_exception(e):
	...
	return APIException.internal_server_error(e)
...
def login(payload: LoginRequest):
	...
	if password == correct:
		...
		raise APIException.unauthorized("Invalid credentials")
...
def list_questions():
	...
	if position >= 0:
		...
		if not questions:
			raise APIException.not_found("Question by position", position)
...
def get_question(question_id: int):
	...
	if not question:
		raise APIException.not_found("Question", question_id)
...
def participate(payload: Participation):
	...
	if number_of_answers != number_of_questions:
		raise APIException(f"Incorrect number of answers, expected {number_of_questions} but received {number_of_answers}")
	correct_answers = 0
	summary = []
	for i in range(number_of_questions):
		...
		if answer <= 0 or answer > number_of_possible_answers:
			raise APIException(f"Invalid answer #{answer} for question #{question.id} (must be between 1 and {number_of_possible_answers})")
		correct = 0
